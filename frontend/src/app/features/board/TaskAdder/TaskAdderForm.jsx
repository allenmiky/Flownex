...
export default function TaskAdderForm({ onClose, defaultStatus = "todo" }) {
	...
	return (
		<>
			...
			{/* Modal Container with Dynamic Width/Height for Full Screen */}
			<div className={`fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--bg-primary)] border-2 border-[var(--accent)] rounded-xl shadow-2xl z-[70] flex flex-col overflow-hidden transition-all duration-300 
				...
				<div className="p-6 pb-2 flex justify-between items-start">
					<div className="flex-1">
						...
						/>
						<p className="text-sm text-[var(--text-secondary)] mt-1">in list <span className="underline">{defaultStatus}</span></p>
					</div>
				...
				<div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar">
					<div className="flex flex-col md:flex-row gap-8 items-start relative">
						...
						<div className="flex-[3] w-full space-y-8">
							...
							{/* Labels & Members Section (Show Side by Side) */}
								...
								{tags.length > 0 && (
									<div>
										...
										<div className="flex gap-2 flex-wrap">
											{tags.map((t, idx) => (
												<span key={t.id || idx} className="h-8 px-3 rounded flex items-center text-white text-xs font-bold" style={{ background: t.color }}>{t.name}</span>
											))}
										</div>
									</div>
								)}

								{/* Members (Added Feature) */}
								{assignee && (
									<div>
				...
				{/* Footer */}
				<div className="p-6 border-t flex justify-end gap-3 bg-[var(--bg-primary)]">
					<button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-[var(--text-secondary)]">Cancel</button>
					<button
						onClick={addTask}
						disabled={!title.trim()}
